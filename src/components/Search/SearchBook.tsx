...
export default function BookSearch({ onSearch }: BookSearchProps) {
  ...
  const fetchBooks = async () => {
    if (!query.trim()){
      onSearch(false);return; //om sökfältet inte har aktiverats visas RandomBöcker
    }
    ...
    try {
      ...
      if (data.docs) {setBooks(data.docs.map((book: any) => ({
            title: book.title,
    ...
    <div className="flex flex-col items-center w-full">
      ...
      >
        ...
        <div className="relative w-full max-w-lg">
          ...
          />
          <button
            onClick={fetchBooks} className="absolute right-4 top-1/2 transform -translate-y-1/2">
            ...
            {books.map((book: any, index) => (
              <li key={index} className="flex flex-col items-center w-40 h-65 bg-white p-4 shadow-[0px_0px_4px_3px_rgba(195,186,171,0.3)]">
                <img src={book.cover_i ? `https://covers.openlibrary.org/b/id/${book.cover_i}-L.jpg` : "/Img/UnDefinedBook.png"}
                  alt={book.title} className="w-40 h-60 object-cover mb-2 rounded-md truncate" />
